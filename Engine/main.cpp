...
public:
	...
	// ë³´ì´ëŠ” ê°ì²´ì™€ ë³´ì´ì§€ ì•ŠëŠ” ê°ì²´ë¥¼ ë¶„ë¥˜í•˜ëŠ” í•¨ìˆ˜
	...
	// ë³´ì´ì§€ ì•ŠëŠ” ê°ì²´ë“¤ì„ ì‚­ì œí•˜ëŠ” í•¨ìˆ˜
	void RemoveInvisiblePrimitives(const std::vector<int>& invisibleIndices)
	{
		...
		for (int i = invisibleIndices.size() - 1; i >= 0; i--)
		{
			...
			if (deleteIndex < 0 || deleteIndex >= Size) continue; // ì•ˆì „ì„± ì²´í¬
			
			delete primitives[deleteIndex];
			
			// ë§ˆì§€ë§‰ ìš”ì†Œë¥¼ ì‚­ì œí•  ìœ„ì¹˜ë¡œ ì´ë™
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	while (bIsExit == false)
	{
		...
		// Dirty í”Œë˜ê·¸ ì—…ë°ì´íŠ¸ ë° ë³´ì´ì§€ ì•ŠëŠ” ê°ì²´ ì‚­ì œ
		std::vector<int> invisiblePrimitives; // ì‚­ì œí•  ê°ì²´ë“¤ì˜ ì¸ë±ìŠ¤
		
		// FPrimitiveVectorì˜ í•¨ìˆ˜ë¥¼ ì‚¬ìš©í•˜ì—¬ ê°€ì‹œì„± ë¶„ë¥˜
		PrimitiveVector.ClassifyVisibility(cam, visiblePrimitives, invisiblePrimitives);
		
		// ë³´ì´ì§€ ì•ŠëŠ” ê°ì²´ë“¤ ì‚­ì œ
